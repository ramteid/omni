...
import { SourceType, type PaperlessSourceConfig } from '$lib/types'
...
export const load: PageServerLoad = async ({ params, locals }) => {
    ...
    // Check whether credentials exist (we never expose the API key itself).
...
export const actions: Actions = {
    default: async ({ request, params, locals, fetch }) => {
        ...
        const url = (formData.get('url') as string | null)?.trim().replace(/\/$/, '') ?? ''
        ...
        try {
            const config: PaperlessSourceConfig = {
                url,
                sync_enabled: isActive,
            }

            await updateSourceById(source.id, { isActive, config: config as unknown as Record<string, unknown> })

            // Update the API key only if a new one was supplied.
            if (apiKey) {
