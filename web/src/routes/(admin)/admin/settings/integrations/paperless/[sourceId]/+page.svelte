<script lang="ts">
    ...
    import { ArrowLeft, Loader2, FileText, Trash2 } from '@lucide/svelte'
    import RemoveSourceDialog from '../../remove-source-dialog.svelte'
    import { onMount } from 'svelte'
    import { beforeNavigate } from '$app/navigation'
    import type { PageProps } from './$types'
    import type { PaperlessSourceConfig } from '$lib/types'

    let { data }: PageProps = $props()

    const cfg = (data.source.config as Partial<PaperlessSourceConfig>) ?? {}
    ...
    const originalEnabled = data.source.isActive
            ...
            <Card.Root class="relative">
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <Card.Description class="mt-1">
                                Index documents and OCR content from your paperless-ngx instance.
                            </Card.Description>
                        </div>
                ...
                <Card.Content class="space-y-3">
                    <div class="text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                        <span class="font-medium">Instance URL</span>
                        <span class="truncate">{cfg.base_url ?? '—'}</span>
                    </div>
                    <p class="text-muted-foreground text-sm">
                        All documents available in this paperless-ngx instance will be indexed,
                        including their OCR text, tags, correspondents, and document types.
                    </p>
                </Card.Content>
