...
if [ "$START_SERVICES" = true ]; then
    ...
    docker compose -f docker/docker-compose.yml -f docker/docker-compose.benchmark.yml -f docker/docker-compose.gpu.yml --env-file .env up -d
...
if ! curl -sf http://localhost:3001/health > /dev/null 2>&1; then
    ...
    echo "Please start services with: docker compose -f docker/docker-compose.yml -f docker/docker-compose.benchmark.yml -f docker/docker-compose.gpu.yml --env-file .env up -d"
    exit 1
fi
