...
pub fn build_manifest(connector_url: String) -> ConnectorManifest {
    ConnectorManifest {
        ...
        description: Some(
            "Index documents from a Paperless-ngx document management system".to_string(),
        ),
        actions: vec![],
...
fn build_client(request: &ActionRequest) -> anyhow::Result<PaperlessClient> {
    ...
    PaperlessClient::new(config.base_url(), api_key)
}
