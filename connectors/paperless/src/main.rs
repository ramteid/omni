use anyhow::Result;
use dotenvy::dotenv;
use omni_paperless_connector::api::{build_manifest, create_router, ApiState};
use omni_paperless_connector::sync::SyncManager;
...
async fn main() -> Result<()> {
    ...
    let telemetry_config = TelemetryConfig::from_env("omni-paperless-connector");
    telemetry::init_telemetry(telemetry_config)?;

    info!("Starting Paperless-ngx Connector");
