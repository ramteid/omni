use anyhow::Result;
use dotenvy::dotenv;
use omni_nextcloud_connector::api::{build_manifest, create_router, ApiState};
use omni_nextcloud_connector::sync::SyncManager;
...
async fn main() -> Result<()> {
    dotenv().ok();
    let telemetry_config = TelemetryConfig::from_env("omni-nextcloud-connector");
