...
async def test_bad_token_fails_sync(
    ...
    mock_hubspot_api.should_fail_auth = True
    ...
    try:
        ...
        mock_hubspot_api.should_fail_auth = False


async def test_missing_token_fails_sync(
    harness, seed, mock_hubspot_server, cm_client: httpx.AsyncClient
):
    sid = await seed.create_source(
        source_type="hubspot",
        config={"api_url": mock_hubspot_server, "portal_id": "12345678"},
    )
    await seed.create_credentials(sid, {}, provider="hubspot")
    ...
    assert (
        ...
        or "access_token" in row["error_message"].lower()
