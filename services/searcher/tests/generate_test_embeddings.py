...
def main():
    ...
    doc_response = generate_embeddings(document_texts, task="retrieval.passage")
    
    # Generate embeddings for queries
    query_response = generate_embeddings(test_queries, task="retrieval.query")
    ...
    for i, doc in enumerate(test_documents):
        if i < len(doc_response["embeddings"]):
            test_data["documents"].append({
                ...
                "chunks": doc_response["chunks"][i] if i < len(doc_response["chunks"]) else [],
                "chunks_count": doc_response["chunks_count"][i] if i < len(doc_response["chunks_count"]) else 0
            })
    ...
    for i, query in enumerate(test_queries):
        if i < len(query_response["embeddings"]):
            test_data["queries"].append({
                "text": query,
                "embedding": query_response["embeddings"][i]
            })
