...
def extract_text_from_pdf(pdf_bytes: bytes) -> PDFExtractionResponse:
    ...
    try:
        ...
        for page_num, page in enumerate(pdf_reader.pages, 1):
            try:
                ...
                if page_text:
                    ...
                    logger.debug(f"Extracted {len(page_text)} characters from page {page_num}/{page_count}")
            except Exception as e:
        ...
        if not extracted_text.strip():
            logger.warning("No text content extracted from PDF - might be a scanned document")
            return PDFExtractionResponse(
                text="",
