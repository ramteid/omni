...
async def test_basic_embedding():
    ...
    try:
        ...
        for i, emb in enumerate(embeddings):
            print(
                f"Text {i+1}: embedding dimension = {len(emb)}, first 5 values = {emb[:5]}"
            )
    ...
    for mode in modes:
        ...
        try:
            ...
            for i, chunk in enumerate(result[0]):
                print(
                    f"    Chunk {i+1}: span={chunk.span}, embedding_dim={len(chunk.embedding)}"
                )
...
def test_sync_wrapper():
    ...
    try:
        ...
        if result[0]:
            print(
                f"First chunk: span={result[0][0].span}, embedding_dim={len(result[0][0].embedding)}"
            )
