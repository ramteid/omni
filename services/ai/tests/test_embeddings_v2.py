...
class TestChunkingSentences:
    ...
    def test_basic_chunking(self, model_and_tokenizer):
        ...
        assert len(char_spans[0]) == 1  # 4 sentences with chunk_size=50 produces 1 chunk
    ...
    def test_chunk_span_validation(self, model_and_tokenizer):
        ...
        _, tokenizer = model_and_tokenizer
        texts = ["First sentence. Second sentence. Third sentence. Fourth sentence. Fifth sentence."]
        inputs = tokenize(tokenizer, texts)
    ...
    def test_different_chunk_sizes(self, model_and_tokenizer):
        ...
        _, tokenizer = model_and_tokenizer
        texts = ["Sentence one. Sentence two. Sentence three. Sentence four. Sentence five."]
        inputs = tokenize(tokenizer, texts)
...
class TestChunkingKSentences:
    ...
    def test_k_sentence_chunking(self, model_and_tokenizer):
        ...
        char_spans, token_spans = generate_sentence_chunks(inputs, tokenizer, k_sentences=3)
        
        assert len(char_spans) == 1
        assert len(token_spans) == 1
        assert len(char_spans[0]) == 2  # 6 sentences with k_sentences=3 produces 2 chunks
        assert len(token_spans[0]) == len(char_spans[0])
    ...
    def test_different_k_values(self, model_and_tokenizer):
        ...
        assert len(char_spans_k2[0]) >= len(char_spans_k5[0])  # Smaller K creates more chunks
    ...
    def test_k_larger_than_sentences(self, model_and_tokenizer):
        ...
        char_spans, token_spans = generate_sentence_chunks(inputs, tokenizer, k_sentences=5)
...
class TestLateChunking:
    ...
    def test_different_batch_sizes(self, model_and_tokenizer):
        ...
        _, token_spans_single = chunk_by_sentences(inputs_single, tokenizer, chunk_size=50)
        chunk_embeddings_single = apply_late_chunking(embeddings_single, token_spans_single)
        ...
        _, token_spans_multi = chunk_by_sentences(inputs_multi, tokenizer, chunk_size=50)
        chunk_embeddings_multi = apply_late_chunking(embeddings_multi, token_spans_multi)
...
class TestSemanticCorrectness:
    ...
    def test_semantic_search_accuracy(self, diverse_document, semantic_queries):
        ...
        test_cases = [
            ("How long should I cook spaghetti?", "cooking", 0.60),  # Should match cooking content with high similarity
            ("Which planet is closest to the Sun?", "solar_system", 0.50),  # Should match solar system content  
            ("When should I plant tomatoes?", "gardening", 0.35),  # Should match gardening content
        ]
        ...
        for query, expected_topic, min_similarity in test_cases:
            ...
            for i, (chunk_idx, similarity) in enumerate(zip(top_k_indices, top_k_similarities)):
                print(f"  {i+1}. Similarity: {similarity:.3f}")
                print(f"     Content: {top_chunks_content[i]}")
            print("-" * 80)
            
            # Assert that top similarity meets minimum threshold
            assert top_k_similarities[0].item() >= min_similarity, \
            ...
            if expected_topic == "cooking":
                assert any(word in top_chunk_content for word in ["pasta", "cook", "boil", "drain"]), \
                    f"Top chunk for cooking query doesn't contain cooking terms: {top_chunk_content[:100]}"
            elif expected_topic == "solar_system":
                assert any(word in top_chunk_content for word in ["solar", "planet", "sun", "mercury", "jupiter"]), \
                    f"Top chunk for solar system query doesn't contain space terms: {top_chunk_content[:100]}"
                pass
            elif expected_topic == "gardening":
                assert any(word in top_chunk_content for word in ["garden", "plant", "soil", "crop", "tomato"]), \
    ...
    def test_semantic_search_with_larger_chunks(self, diverse_document):
        ...
        assert len(chunks) < 8, f"Expected fewer than 8 chunks with larger chunk size, got {len(chunks)}"
        assert len(chunks) > 0, "Should have at least one chunk"
        
        # Should still find relevant cooking content
        assert best_similarity > 0.5, f"Expected similarity > 0.5 for cooking query, got {best_similarity:.3f}"
        
        # Verify the best matching chunk contains cooking content
        best_chunk_span = chunks[best_chunk_idx].span
        best_chunk_content = diverse_document[best_chunk_span[0]:best_chunk_span[1]].lower()
        assert any(word in best_chunk_content for word in ["pasta", "cook", "boil"]), \
...
class TestEdgeCases:
    ...
    def test_long_text_chunking(self):
        ...
        assert len(result) == 1
        assert len(result[0]) == 225  # 1000 sentences with chunk_size=200 produces 225 chunks
...
class TestPerformance:
    ...
    def test_chunk_span_alignment(self, model_and_tokenizer):
        ...
        for (char_start, char_end), (token_start, token_end) in zip(char_spans[0], token_spans[0]):
    ...
    def test_embedding_dimensions(self):
        ...
        texts = ["Short.", "A longer sentence with more words.", "Text with different length and complexity."]
    ...
    def test_batch_processing_consistency(self):
        ...
        for batch_chunks, individual_chunks in zip(batch_result, individual_results):
            ...
            for batch_chunk, individual_chunk in zip(batch_chunks, individual_chunks):
                assert np.allclose(batch_chunk.embedding, individual_chunk.embedding, atol=1e-8)
